// input.js - Harmonized Vector Polygon Plotting, Line Roads, and Continuous Drag-Painting
...
let isPanning = false;
let isContinuousPainting = false; // Active drag paint tracker
...
export function initInputHandlers(canvas, onPaint) {
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Escape') { activePolygonVertices = []; startGridPos = null; }
    });

    window.addEventListener('keyup', (e) => {
        if (e.key === 'Shift') { isDrawingMode = false; startGridPos = null; }
    });

    canvas.addEventListener('mousedown', (e) => {
        const gridPos = getTargetGridCell(e, canvas);

        if (e.button === 1 || (e.button === 0 && !isDrawingMode && (currentBrush === "ROAD" || currentBrush === "BULLDOZER") && !e.ctrlKey)) {
            // Enable dragging/panning if not executing a specific action path
            if (e.ctrlKey === false && e.button === 0 && (currentBrush === "ROAD" && !isDrawingMode || currentBrush === "BULLDOZER")) {
                isContinuousPainting = true; // Turn on mousemove structural adjustments
            ...
            if (currentBrush === "ROAD") {
                if (isDrawingMode) {
                    if (!startGridPos) { startGridPos = gridPos; } 
                    else {
            ...
            else if (currentBrush === "BULLDOZER") {
                isContinuousPainting = true;
                if (onPaintCallback) onPaintCallback(gridPos.x, gridPos.y, null);
            }
    ...
    canvas.addEventListener('mousemove', (e) => {
        ...
        // Execute continuous sweeping operations
        if (isContinuousPainting && (currentBrush === "ROAD" || currentBrush === "BULLDOZER") && !isDrawingMode) {
            if (onPaintCallback) onPaintCallback(rawGridPos.x, rawGridPos.y, null);
        }
...
export function setSystemBrush(brushString) {
    ...
    activePolygonVertices = []; startGridPos = null;
    isContinuousPainting = false;
}
...
export function getActivePlotVertices() { return activePolygonVertices; }
...
export function getPlacementGhost() {
    if (currentGridPos && (currentBrush === "POWER_PLANT" || currentBrush === "BULLDOZER" || (currentBrush === "ROAD" && !isDrawingMode))) {
